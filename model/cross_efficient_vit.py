...
class se1d(nn.Module):
    def __init__(self, length, ratio) -> None:
        super().__init__()
        self.dim = length
        self.ratio = ratio
        self.squeeze = nn.Sequential(
            nn.AvgPool1d(kernel_size=self.dim)
        )
        self.excitation = nn.Sequential(
            nn.Linear(in_features=2, out_features=2*ratio, bias=True),
            nn.ReLU(),
            nn.Linear(in_features=2*ratio, out_features=2, bias=True),
            nn.ReLU()
        )
import yaml
with open('E:\\study\\code\\Combining-EfficientNet-and-Vision-Transformers-for-Video-Deepfake-Detection-main\\cross-efficient-vit\\configs\\architecture.yaml', 'r') as ymlfile:
